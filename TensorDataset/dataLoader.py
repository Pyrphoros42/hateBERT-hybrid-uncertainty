...
def combine_features(tuple_data,params,is_train=False):
    ...
    try:
        ...
        if params['AU']:
            dissent = [ele [3] for ele in tuple_data]
        elif params['EU']:
            # if target contains 'Hispanic' or 'Refugee' = 1, else 0
            target = [ele[3] for ele in tuple_data]
    ...
    if params['AU']:
        dataloader=return_mod_dataloader(input_ids,labels,att_vals,att_masks,dissent,params,is_train)
        return dataloader
    elif params['EU']:
        dataloader=return_mod_dataloader(input_ids,labels,att_vals,att_masks,target,params,is_train)
        return dataloader
    else:
...
def return_mod_dataloader(input_ids,labels,att_vals,att_masks,modified,params,is_train=False):
    ...
    data = TensorDataset(inputs,attention,masks,labels, modified)
    if(is_train==False):
